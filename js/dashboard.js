...
window.Dashboard = {
  ...
  /**
   * Dashboard sahifasini render qilish
   */
      ...
      <div class="container" style="padding-top: 32px;">
        <div class="dashboard-welcome">
          <div class="dashboard-welcome-content">
            <h1>
              ${this._getGreeting()}, ${Auth.getDisplayName(user)}! 👋
            </h1>
        ...
        <!-- Bottom: Time spent / Weekly chart -->
        <div class="card mt-3">
          <div class="card-header">
  ...
  _renderQuizStats(quizResults, courses) {
    ...
    if (results.length === 0) {
      return `
        <div style="text-align: center; padding: 24px; color: var(--text-muted);">
          ...
          <button class="btn btn-sm btn-green mt-2" data-page="courses">
            Kurslarni ko'rish
          </button>
        </div>
      `;
    ...
    return `
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 16px;">
        ...
        <div style="text-align: center; padding: 16px; background: var(--bg-elevated); border-radius: var(--radius-md);">
          <div style="font-size: 24px; font-weight: 800; color: var(--danger);">
            ${failed}
          </div>
          <div style="font-size: 12px; color: var(--text-muted);">O'tilmagan</div>
        </div>
        <div style="text-align: center; padding: 16px; background: var(--bg-elevated); border-radius: var(--radius-md);">
          <div style="font-size: 24px; font-weight: 800; color: var(--excel-green);">
            ${avgScore}%
          </div>
          <div style="font-size: 12px; color: var(--text-muted);">O'rtacha ball</div>
        </div>
        <div style="text-align: center; padding: 16px; background: var(--bg-elevated); border-radius: var(--radius-md);">
          <div style="font-size: 24px; font-weight: 800; color: var(--color-dashboard);">
            ${Math.floor(avgTime / 60)}m
          </div>
          <div style="font-size: 12px; color: var(--text-muted);">O'rtacha vaqt</div>
        </div>
      </div>
  ...
  _getRecentActivity(progress, quizResults, certificates) {
    ...
    return activities.slice(0, 10); // Faqat 10 ta
  },
  ...
  _renderAchievements(stats) {
    const achievements = [
      ...
      { 
        ...
        icon: '📚',
        title: 'O\'rganuvchi',
        desc: '5 ta darsni tugatish',
        unlocked: stats.completedLessons >= 5
      },
      { 
        id: 'first_quiz',
        icon: '🎯',
        title: 'Test qiruvchisi',
        desc: '1 ta testdan o\'tish',
        unlocked: stats.quizCount >= 1
      },
      { 
        id: 'first_cert',
        icon: '🏆',
        title: 'Sertifikatli',
        desc: '1 ta sertifikat olish',
        unlocked: stats.certificatesCount >= 1
      },
      { 
        id: 'half_progress',
        icon: '⭐',
      ...
      { 
        ...
        title: 'Master',
        desc: '100% kurs',
        unlocked: stats.overallProgress >= 100
      }
    ];
...
console.log('📊 Dashboard modul yuklandi');