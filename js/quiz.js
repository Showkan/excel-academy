...
window.Quiz = {
        ...
        <div class="quiz-container">
          <div class="card">
            <div class="card-body" style="padding: 40px; text-align: center;">
              ...
              <p style="color: var(--text-secondary); margin-bottom: 32px;">${I18n.t('quiz_title')}</p>

              <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 32px;">
                <div style="background: var(--bg-elevated); padding: 20px; border-radius: var(--radius-md);">
                  <div style="font-size: 28px; font-weight: 800; color: var(--excel-green);">${quiz.questions.length}</div>
                  <div style="font-size: 13px; color: var(--text-muted);">Savollar</div>
                </div>
                <div style="background: var(--bg-elevated); padding: 20px; border-radius: var(--radius-md);">
                  <div style="font-size: 28px; font-weight: 800; color: var(--color-dashboard);">${quiz.timeLimit || '∞'}</div>
                  <div style="font-size: 13px; color: var(--text-muted);">Daqiqa</div>
                </div>
                <div style="background: var(--bg-elevated); padding: 20px; border-radius: var(--radius-md);">
                  <div style="font-size: 28px; font-weight: 800; color: var(--color-bonus);">${quiz.passingScore || 70}%</div>
              ...
              <div style="background: var(--bg-elevated); padding: 20px; border-radius: var(--radius-md); margin-bottom: 24px; text-align: left;">
                ...
                <ul style="display: flex; flex-direction: column; gap: 8px; font-size: 13px; color: var(--text-secondary); padding-left: 20px;">
                  ...
                  <li>O'tish uchun kamida ${quiz.passingScore || 70}%</li>
                  <li>Qayta topshirish mumkin</li>
                  <li>O'tib bo'lsa <strong>+${typeof XPSystem !== 'undefined' ? XPSystem.rewards.testPass : 25} XP</strong> olasiz</li>
                </ul>
              </div>
  ...
  _startTimer() {
    ...
    this.timer = setInterval(() => {
      ...
      if (timerEl) timerEl.textContent = this._formatTime(this.elapsedSeconds);
    }, 1000);
  },
    ...
    container.innerHTML = `
      <div class="quiz-page">
        <div class="quiz-container">
          ...
          <div class="quiz-card">
            <div class="quiz-question-num">${I18n.t('quiz_question')} ${this.currentQuestionIndex + 1}</div>
            ...
            <div class="quiz-options">
              ${question.options.map((option, idx) => `
                <button class="quiz-option ${userAnswer === idx ? 'selected' : ''}" onclick="Quiz.selectAnswer(${idx})">
            ...
            <div class="quiz-actions">
              <button class="btn btn-outline" onclick="Quiz.prevQuestion()" ${this.currentQuestionIndex === 0 ? 'disabled' : ''}>
              ...
              <div style="display: flex; gap: 8px;">
                ${quiz.questions.map((_, idx) => {
                  ...
                  return `
                    <button onclick="Quiz.goToQuestion(${idx})" style="
  ...
  async submit() {
    ...
    if (unanswered > 0) {
      if (!confirm(`${unanswered} ta savolga javob berilmagan. Davom etasizmi?`)) return;
    }
    ...
    if (Auth.isLoggedIn()) {
      try {
        await FB.saveQuizResult(Auth.currentUser.uid, this.currentCourse.id, this.currentLesson.id, result);

        if (result.passed) {
          await FB.saveProgress(Auth.currentUser.uid, this.currentCourse.id, this.currentLesson.id, {
            type: 'quiz_passed',
        ...
        if (typeof XPSystem !== 'undefined') {
          XPSystem.addTest(result.percentage);
        }
      } catch (error) {
  ...
  _calculateResult() {
    ...
    return {
      score: correct, total, percentage, passed, passingScore,
      timeSpent: this.elapsedSeconds, answers: details
    };
  },
...
console.log('%c🎯 Quiz yuklandi', 'color: #2b7de9; font-weight: bold;');
